...
const convertPdfToImage = async (pdfPath) => {
  ...
  const pageCount = await pdf2pic.convert(pdfPath, { page: 1 });
  return path.join(outputImagePath, `${pageCount.page}.jpg`);
};

// Function to use Vision API on the converted image
...
// Route to handle PDF conversion and image analysis
app.get('/process-pdf', async (req, res) => {
  ...
  try {
    await downloadFile(fileId, pdfPath);
